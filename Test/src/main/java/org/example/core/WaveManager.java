...
//Experiment: para dili lag ang duwa
package org.example.core;
...
public class WaveManager {
    ...
    private void scheduleEnemySpawning() {
        spawnTask = () -> {
            ...
            if (currentWave > 5 && random.nextDouble() < 0.2) { // Reduced probability from 0.3 to 0.2
                spawnCount = Math.min(currentWave / 4, 3); // Reduced max spawnCount from 4 to 3
            }
        ...
        FXGL.getGameTimer().runAtInterval(spawnTask, Duration.seconds(0.5)); // Increased from 0.3 to 0.5
    }
    ...
    private void spawnEnemyForCurrentWave(int survivalTime) {
        ...
        int baseMax = waveMaxEnemies.getOrDefault(currentWave, BASE_ENEMIES_PER_WAVE);
        double timeScaling = 1.0 + (survivalTime / 600.0); // Reduced scaling factor
        int maxEnemies = (int)(baseMax * timeScaling);

        int currentEnemyCount = FXGL.getGameWorld().getEntitiesByType(EntityType.ENEMY).size();
        int waveAllowance = Math.min(currentWave * 3, 30); // Reduced from *5, 50 to *3, 30
        ...
        String enemyType;
        if (currentWave >= 6 && random.nextDouble() < 0.05 * (currentWave / 6.0)) { // Reduced boss spawn chance
            int bossIndex = Math.min(enemyPool.size() - 1, enemyPool.size() - 2);
            enemyType = enemyPool.get(Math.max(bossIndex, 0));
        } else {
            enemyType = enemyPool.get(random.nextInt(enemyPool.size()));
    ...
    private void cleanupOldEnemies() {
        ...
        double viewMaxX = viewMinX + FXGL.getAppWidth();
        double viewMaxY = viewMinY + FXGL.getAppHeight();
        double margin = 1000; // Extended margin for cleanup

        List<Entity> enemies = FXGL.getGameWorld().getEntitiesByType(EntityType.ENEMY);
        for (Entity enemy : enemies) {
            double x = enemy.getX();
            double y = enemy.getY();
            long lifetime = System.nanoTime() - enemy.getObject("spawnTime", System.nanoTime());
            ...
            if (isOld && (x < viewMinX - margin || x > viewMaxX + margin || y < viewMinY - margin || y > viewMaxY + margin)) {
                ...
                System.out.println("Removed old enemy at (" + x + ", " + y + ") to reduce lag");
            }
        }
    }
    ...
    private void announceNewWave() {
        ...
        if (currentWave % 5 == 0) {
            ...
            }, Duration.seconds(2)); // Increased delay to 2s
        }
        ...
        if (currentWave % 10 == 0) {
            ...
            // Stagger formations over time
            FXGL.getGameTimer().runOnceAfter(() -> {
                String enemyType = waveEnemyPools.get(currentWave).get(
                        random.nextInt(waveEnemyPools.get(currentWave).size()));
                System.out.println("Spawning first formation (circle) for Wave " + currentWave + " with enemy: " + enemyType);
                spawnEnemyInCircleFormation(enemyType);
            }, Duration.seconds(3)); // Increased delay

            // Reduced to two formations instead of three
            FXGL.getGameTimer().runOnceAfter(() -> {
                String secondEnemyType = waveEnemyPools.get(currentWave).get(
                        random.nextInt(waveEnemyPools.get(currentWave).size()));
                System.out.println("Spawning second formation (spiral) for Wave " + currentWave + " with enemy: " + secondEnemyType);
                spawnEnemyInSpiralFormation(secondEnemyType);
            }, Duration.seconds(6)); // Increased delay
        }
        System.out.println("=========================================");
    }
    ...
    private void spawnEnemyOutsideViewport(String enemyType) {
        ...
        enemy.setProperty("spawnTime", System.nanoTime()); // Track spawn time for cleanup
    }
    ...
    private void spawnEnemyInCircleFormation(String enemyType) {
        double radius = 800;
        int count = 8; // Reduced from 12

        if (currentWave > 5) {
            count = 8 + Math.min((currentWave - 5), 8); // Max 16 instead of 24
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            }, Duration.millis(100 * i)); // 100ms delay per enemy
        }
    }
    ...
    private void spawnEnemyInLineFormation(String enemyType) {
        double distance = 800;
        int count = 6; // Reduced from 8
        double spacing = 80;

        if (currentWave > 5) {
            count = 6 + Math.min((currentWave - 5), 4); // Max 10 instead of 15
        }
        ...
        for (int i = 0; i < count; i++) {
            final int index = i;
            FXGL.getGameTimer().runOnceAfter(() -> {
                double x = startX + dirX * spacing * index;
                double y = startY + dirY * spacing * index;

                SpawnData data = new SpawnData(x, y);
                data.put("player", player);
                Entity enemy = FXGL.getGameWorld().spawn(enemyType, data);
                enemy.setProperty("spawnTime", System.nanoTime());
            }, Duration.millis(150 * i)); // 150ms delay per enemy
        }
    }
     ...
     */
    private void spawnEnemyInSpiralFormation(String enemyType) {
        int count = 10; // Reduced from 16
        double baseRadius = 700;
        double radiusIncrement = 40;

        if (currentWave > 5) {
            count = 10 + Math.min((currentWave - 5), 10); // Max 20 instead of 32
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            FXGL.getGameTimer().runOnceAfter(() -> {
                ...
                Entity enemy = FXGL.getGameWorld().spawn(enemyType, data);
                enemy.setProperty("spawnTime", System.nanoTime());
            }, Duration.millis(120 * i)); // 120ms delay per enemy
        }
    }
}